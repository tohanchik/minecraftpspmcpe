...
float TileRenderer::getWaterCornerHeight(int wx, int wy, int wz) const {
  if (m_level->isWater(wx, wy + 1, wz)) return 1.0f;
  if (!m_level->isWater(wx, wy, wz)) return 0.0f;
  return 0.8889f;
}

bool TileRenderer::tesselateWaterInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  int wxi = cx * CHUNK_SIZE_X + lx;
  int wyi = ly;
  int wzi = cz * CHUNK_SIZE_Z + lz;
  float wx = (float)wxi;
  float wy = (float)wyi;
  float wz = (float)wzi;

  bool up = !m_level->isWater(wxi, wyi + 1, wzi);
  bool down = !g_blockProps[m_level->getBlock(wxi, wyi - 1, wzi)].isLiquid();
  bool north = !g_blockProps[m_level->getBlock(wxi, wyi, wzi - 1)].isLiquid();
  bool south = !g_blockProps[m_level->getBlock(wxi, wyi, wzi + 1)].isLiquid();
  bool west = !g_blockProps[m_level->getBlock(wxi - 1, wyi, wzi)].isLiquid();
  bool east = !g_blockProps[m_level->getBlock(wxi + 1, wyi, wzi)].isLiquid();
  if (!up && !down && !north && !south && !west && !east) return false;

  const BlockUV &uv = g_blockUV[id];
  const float ts = 1.0f / 16.0f;
  const float eps = 0.125f / 256.0f;
  const uint32_t waterBaseTop = 0xA0F0C060;
  const uint32_t waterBaseSide = 0x90E0B050;

  Tesselator *t = m_transTess;
  bool drawn = false;

  float hNW = getWaterCornerHeight(wxi, wyi, wzi);
  float hSW = getWaterCornerHeight(wxi, wyi, wzi + 1);
  float hSE = getWaterCornerHeight(wxi + 1, wyi, wzi + 1);
  float hNE = getWaterCornerHeight(wxi + 1, wyi, wzi);

  if (up) {
    float sl00 = getVertexSkyLight(wxi, wyi + 1, wzi, -1, 0, 0, 0, 0, -1);
    float sl10 = getVertexSkyLight(wxi, wyi + 1, wzi, 1, 0, 0, 0, 0, -1);
    float sl01 = getVertexSkyLight(wxi, wyi + 1, wzi, -1, 0, 0, 0, 0, 1);
    float sl11 = getVertexSkyLight(wxi, wyi + 1, wzi, 1, 0, 0, 0, 0, 1);
    float bl00 = getVertexBlockLight(wxi, wyi + 1, wzi, -1, 0, 0, 0, 0, -1);
    float bl10 = getVertexBlockLight(wxi, wyi + 1, wzi, 1, 0, 0, 0, 0, -1);
    float bl01 = getVertexBlockLight(wxi, wyi + 1, wzi, -1, 0, 0, 0, 0, 1);
    float bl11 = getVertexBlockLight(wxi, wyi + 1, wzi, 1, 0, 0, 0, 0, 1);
    float avgBlk = (bl00 + bl10 + bl01 + bl11) * 0.25f;
    float avgSky = (sl00 + sl10 + sl01 + sl11) * 0.25f;
    bool useBlk = (avgBlk > avgSky + 0.05f);
    uint32_t c00 = applyLightToFace(waterBaseTop, useBlk ? bl00 : sl00);
    uint32_t c10 = applyLightToFace(waterBaseTop, useBlk ? bl10 : sl10);
    uint32_t c01 = applyLightToFace(waterBaseTop, useBlk ? bl01 : sl01);
    uint32_t c11 = applyLightToFace(waterBaseTop, useBlk ? bl11 : sl11);
    ...
    t->addQuad(u0, v0, u1, v1, c00, c10, c01, c11,
               ...
               wx + 1.0f, wy + hNE, wz,
               wx, wy + hSW, wz + 1.0f,
               wx + 1.0f, wy + hSE, wz + 1.0f);
    drawn = true;
  }

  float u0s = uv.side_x * ts + eps, v0s = uv.side_y * ts + eps;
  float u1s = (uv.side_x + 1) * ts - eps, v1s = (uv.side_y + 1) * ts - eps;

  if (down) {
    float br = getSkyLightRaw(lx, ly, lz, cx, cz, 0, -1, 0);
    uint32_t c = applyLightToFace(waterBaseSide, br * 0.5f);
    t->addQuad(u0s, v0s, u1s, v1s, c, c, c, c,
               wx, wy, wz + 1.0f, wx + 1.0f, wy, wz + 1.0f, wx, wy, wz, wx + 1.0f, wy, wz);
    drawn = true;
  }
  if (north) {
    float br = getSkyLightRaw(lx, ly, lz, cx, cz, 0, 0, -1) * 0.8f;
    uint32_t c = applyLightToFace(waterBaseSide, br);
    t->addQuad(u0s, v0s, u1s, v1s, c, c, c, c,
               wx + 1.0f, wy + hNE, wz, wx, wy + hNW, wz, wx + 1.0f, wy, wz, wx, wy, wz);
    drawn = true;
  }
  if (south) {
    float br = getSkyLightRaw(lx, ly, lz, cx, cz, 0, 0, 1) * 0.8f;
    uint32_t c = applyLightToFace(waterBaseSide, br);
    t->addQuad(u0s, v0s, u1s, v1s, c, c, c, c,
               wx, wy + hSW, wz + 1.0f, wx + 1.0f, wy + hSE, wz + 1.0f, wx, wy, wz + 1.0f, wx + 1.0f, wy, wz + 1.0f);
    drawn = true;
  }
  if (west) {
    float br = getSkyLightRaw(lx, ly, lz, cx, cz, -1, 0, 0) * 0.6f;
    uint32_t c = applyLightToFace(waterBaseSide, br);
    t->addQuad(u0s, v0s, u1s, v1s, c, c, c, c,
               wx, wy + hNW, wz, wx, wy + hSW, wz + 1.0f, wx, wy, wz, wx, wy, wz + 1.0f);
    drawn = true;
  }
  if (east) {
    float br = getSkyLightRaw(lx, ly, lz, cx, cz, 1, 0, 0) * 0.6f;
    uint32_t c = applyLightToFace(waterBaseSide, br);
    t->addQuad(u0s, v0s, u1s, v1s, c, c, c, c,
               wx + 1.0f, wy + hSE, wz + 1.0f, wx + 1.0f, wy + hNE, wz, wx + 1.0f, wy, wz + 1.0f, wx + 1.0f, wy, wz);
    drawn = true;
  }
...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (id == BLOCK_WATER_STILL || id == BLOCK_WATER_FLOW) {
    return tesselateWaterInWorld(id, lx, ly, lz, cx, cz);
  }
