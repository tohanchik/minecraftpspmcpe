...
bool TileRenderer::tesselateWaterInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (up) {
    ...
    float topL = (useBlk ? (bl00 + bl10 + bl01 + bl11) : (sl00 + sl10 + sl01 + sl11)) * 0.25f;
    uint32_t cTop = applyLightToFace(waterBaseTop, topL);

    float u0 = uv.top_x * ts + eps, v0 = uv.top_y * ts + eps;
    float u1 = (uv.top_x + 1) * ts - eps, v1 = (uv.top_y + 1) * ts - eps;
    t->addQuad(u0, v0, u1, v1, cTop, cTop, cTop, cTop,
               wx, wy + hNW, wz,
               wx + 1.0f, wy + hNE, wz,
               wx, wy + hSW, wz + 1.0f,
