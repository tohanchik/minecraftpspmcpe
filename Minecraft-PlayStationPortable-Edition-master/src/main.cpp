...
static void game_update(float dt) {
  ...
  bool inWater = g_level->isAreaInWater(g_player.x, g_player.y, g_player.z, R, H);
  ...
  if (PSPInput_JustPressed(PSP_CTRL_SELECT)) {
    if (g_player.jumpDoubleTapTimer > 0.0f) {
      ...
      if (!g_player.isFlying && (g_player.onGround || inWater)) {
        g_player.velY = 6.5f;
        g_player.onGround = false;
      }
  ...
  static const uint8_t PLACEABLE[] = {
    ...
    BLOCK_GLOWSTONE, BLOCK_PUMPKIN, BLOCK_WATER_STILL,
