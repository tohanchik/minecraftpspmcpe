...
bool Level::hasWaterSupport(int wx, int wy, int wz) const {
  ...
  if (isWater(wx - 1, wy, wz)) return true;
  if (isWater(wx + 1, wy, wz)) return true;
  if (isWater(wx, wy, wz - 1)) return true;
  if (isWater(wx, wy, wz + 1)) return true;
  return false;
}
...
void Level::processWaterStep() {
  ...
  if (id == BLOCK_WATER_FLOW) {
    if (hasWaterSupport(wx, wy, wz)) {
      setBlock(wx, wy, wz, BLOCK_WATER_STILL);
    } else if (!didSpread) {
      setBlock(wx, wy, wz, BLOCK_AIR);
    }
  } else if (id == BLOCK_WATER_STILL && !hasWaterSupport(wx, wy, wz) && !didSpread) {
    setBlock(wx, wy, wz, BLOCK_WATER_FLOW);
  }
}
